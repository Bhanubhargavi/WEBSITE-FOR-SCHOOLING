  ...
  if ($result->num_rows > 0) {
      ...
      foreach ($row as $course) {
          $sql = "SELECT your_correct_time_slot_column_name, course_name FROM courses1 WHERE course_id = $course";
          $result = $conn->query($sql);
          if ($result->num_rows > 0) {
              $row = $result->fetch_assoc();
              $slot = $row["your_correct_time_slot_column_name"];
              $course_names[] = $row['course_name'];
              // Convert the slot to the appropriate exam date
              // Add switch cases for different time slots if needed
              switch ($slot) {
                  case "A1":
